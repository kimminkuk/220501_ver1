...
//        }
    ...
    public String sangjangInit(Model model) throws IOException {
        ...
        List<CompanyCompareClassVer2> company_roe_code = new ArrayList();
        ...
        for (Company company : company_market_code) {
            String companyKey = company.name + " " + company.code;
            company_point_hash.put(companyKey, 0);
            ...
            //ROE
            Elements select_roe = doc_2.select("#content > div.section.trade_compare > table > tbody > tr:nth-child(12) > td:nth-child(2)");
            System.out.println("select_pcr = " + select_roe + " company name: " + company.name);
            company_roe_code.add(new CompanyCompareClassVer2(companyFigureCnvToFloatVer2(select_roe.text()), company.code, company.name));
        ...
        Collections.sort(company_roe_code);
        int temp1 = 1;
        for (CompanyCompareClass companyCompareClass : company_pbr_code) {
            String companyKey = companyCompareClass.name + " " + companyCompareClass.code;
            int temp = company_point_hash.get(companyKey);
            if (companyCompareClass.compareFloat == 99999) {
                company_point_hash.put(companyKey, temp + 500);
            } else {
                company_point_hash.put(companyKey, temp1+temp);
            }
        ...
        for (CompanyCompareClass companyCompareClass : company_per_code) {
            ...
            if (companyCompareClass.compareFloat == 99999) {
                ...
                company_point_hash.put(companyKey, temp2+temp);
            }
        ...
        int roe_init_point = 1;

        for (CompanyCompareClassVer2 companyCompareClass : company_roe_code) {
            String companyKey = companyCompareClass.name + " " + companyCompareClass.code;
            int roe_mid_point = company_point_hash.get(companyKey);
            if (companyCompareClass.compareFloat == -99999) {
                company_point_hash.put(companyKey, roe_mid_point + 500);
            } else {
                company_point_hash.put(companyKey, roe_init_point+roe_mid_point);
            }
            System.out.println("[roe] companyCompareClass.compareFloat = " + companyCompareClass.compareFloat + " company.name : " + companyCompareClass.name);
            roe_init_point = roe_init_point + 1;
        }

        List<Map.Entry<String, Integer>> entries = new ArrayList<>(company_point_hash.entrySet());
        Collections.sort(entries, new Comparator<Map.Entry<String, Integer>>() {
            @Override
            public int compare(Map.Entry<String, Integer> o1, Map.Entry<String, Integer> o2) {
                return o1.getValue().compareTo(o2.getValue()); //오름차순 정렬
            }
        });
        for (Map.Entry<String, Integer> entry : entries) {
            System.out.println("name = " + entry.getKey() +" 점수 = " + entry.getValue());
        }
        return "sangjang";
    }
    ...
    public Float companyFigureCnvToFloatVer2(String company_text) {
        int temp = -99999;
        if (company_text.isEmpty()) {
            return Float.valueOf(temp);
        }
    ...
    static class CompanyCompareClassVer2 implements Comparable<CompanyCompareClassVer2>{
        private float compareFloat;
        private String code;
        private String name;

        public CompanyCompareClassVer2(float compareFloat, String code, String name) {
            this.compareFloat = compareFloat;
            this.code = code;
            this.name = name;
        }
        ...
        //Reverse Sort
        @Override
        public int compareTo(CompanyCompareClassVer2 c) {
            if (this.compareFloat < c.compareFloat) {
                return 1;
            } else if (this.compareFloat > c.compareFloat) {
                return -1;
